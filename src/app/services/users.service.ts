...
export class UsersService {
  ...
  invite(invite: UserInvite) {
    ...
    return this.http.post(environment.apiEndPoint + "/users/invite", invite, {headers, observe: 'response'});
  }

  getInvitedUser(inviteCode: string) {
    return this.http.get<User>(environment.apiEndPoint + `/users/invited/${inviteCode}`, {observe: 'response'});
  }
